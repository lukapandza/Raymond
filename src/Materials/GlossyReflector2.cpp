#include "GlossyReflector2.h"
#include "World/World.h"

GlossyReflector2::GlossyReflector2()
...
GlossyReflector2::GlossyReflector2(const GlossyReflector2& rhs) 
{
	if (rhs.ambient_brdf)
...
GlossyReflector2*
GlossyReflector2::clone() const 
{
	return new GlossyReflector2(*this);
}
...
GlossyReflector2&
GlossyReflector2::operator=(const GlossyReflector2& rhs)
{
	if (this == &rhs)
		return *this;
...
RGBColor
GlossyReflector2::area_light_shade(ShadeRec& sr)
{
...
RGBColor
GlossyReflector2::path_shade(ShadeRec& sr)
{
	/*
	notes:
	...
	*/
	...
	RGBColor f_r = this->reflected_brdf->sample_f(sr, w_o, w_i_r);

	double k_d = this->diffuse_brdf->get_kd();
	double k_s = this->specular_brdf->get_ks();
	double k_r = this->reflected_brdf->get_ks();

	double rand = rand_float(0, 1.0);
	double p_d = k_d / (k_d + k_s + k_r);
	double p_s = k_s / (k_d + k_s + k_r);
	double p_r = k_r / (k_d + k_s + k_r);
	...
	// return (p_d * f_d / k_d + p_s * f_s / k_s + p_r * f_r / k_r) * sr.w.tracer_ptr->trace_ray(Ray(sr.hit_point, w_i), sr.depth + 1);

	return (p_d * f_d + p_s * f_s + p_r * f_r)
...
RGBColor
GlossyReflector2::global_shade(ShadeRec& sr)
{
	RGBColor L(0);
	if (sr.depth == 0)
